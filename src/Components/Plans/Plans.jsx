...
function PlanCard () {
  ...
  return (
    plans.map( (item) => (
      <div key={item.title} className={item.isFavorite ? 'plan-card-favorite' : 'plan-card'}>
        <h3 className='plan-card-title'>{item.title}</h3>
        <button className={item.isFavorite ? 'isfavorite-show' : 'isfavorite-notshow'}>Preferido</button>
